...
- **설계로 연다(열기).** `superpower`로 스펙·계획·성공 기준·테스트를 코드보다 먼저 세운다. 판돈이 작으면 가볍게, 새 기능·큰 변경이면 제대로 밟는다. 앞단 설계가 곧 뒤의 실행 검증에서 "무엇을 통과로 볼지"의 기준이 된다.
...
2. **단순성 렌즈 (과설계).** "지금부터 나는 불필요한 복잡함만 본다." 요청을 푸는 데 필요 없는 추상화·계층·설정·범용화를 넣지 않았는가. 더 단순한 방법이 있는가. "나중에 쓸지도 몰라서" 미리 지은 것은 지운다.
