...
2. **소제목의 집합이 곧 '목차이자 논리 골격'이다 (제일 중요).** 보고는 목적에 맞는 **알아볼 수 있는 논리 골격(framework)**을 따르고, 소제목이 그 골격의 각 자리를 드러내야 한다. 그래서 소제목만 위에서 아래로 훑으면 "아, 이건 **문제를 제기하고 → 원인을 분석하고 → 해결하고 → 재발을 막으려는** 보고구나"처럼 **의도와 흐름이 목차만으로** 잡혀야 한다.

   목적에 맞는 골격을 고른다:
   - **문제 해결·품질 (DRBFM):** 문제 → 긴급조치 → 발생 원인 → 유출 원인 → 해결책 → 예방책. FITogether 표준 양식이며, 하드웨어 설계 변경 보고는 `drbfm-qa` 스킬이 이 골격을 그대로 만들어 준다.
   - **제안·설득 (SCQA):** 상황 → 문제 → 핵심 질문 → 답(권고).
   - **결정 요청:** 결론 → 왜 이 선택인가 → 무엇을 포기했나 → 실행.
   - **진행 공유:** 지금 상태 → 막힌 것 → 다음 단계.

   좋고 나쁨의 기준은 '문장이냐 딱지냐'가 아니라 **'소제목만으로 논리 흐름이 보이느냐'**다. `내용`·`기타`·`분석 3`처럼 역할도 흐름도 안 드러나는 소제목이 나쁜 것이다. 각 절 안에서는 그 절의 핵심을 첫 줄에 두어, 골격(목차)과 결론-먼저가 함께 가게 한다.
