...
2. **결론부터, 그리고 소제목이 곧 스토리다 (제일 중요).** 소제목만 위에서 아래로 훑어도 전체 이야기가 서야 한다. 그러려면 소제목이 주제 딱지가 아니라 **완결된 메시지 문장**이어야 한다.
   - 나쁨(주제 딱지): `매출 분석` / `원인` / `대응`
   - 좋음(메시지): `매출은 3분기에 20% 꺾였다` / `원인은 신규가 아니라 기존 고객 이탈이다` / `이탈 3사만 잡으면 절반이 복구된다`

   소제목만 이어 읽으면 그대로 한 편의 논리가 된다. 이것이 사용자가 세부를 안 봐도 흐름을 파악하는 방법이다.
...
- **보고를 양으로 때우지 않는다.** 소제목이 주제 딱지에 그치거나, 결론이 맨 뒤에 묻히면 실패다.
