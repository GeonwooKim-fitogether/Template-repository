...
- **밴드 3 · 품질 기준선** — 자동 검사별 상태(통과·주의·위반, 그리고 **보고가 없으면 회색 "정보 없음"**)와 걸린 항목, 기획 정합성 막대, 이 보드가 못 보는 것을 적은 한 문단.
...
`.integration/board.config.json`이 있으면 그것을 쓴다. 없으면 번들된 예시 **둘 중 가까운 쪽**을 복사해 그 프로젝트의 어휘로 고친다.
