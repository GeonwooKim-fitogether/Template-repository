...
3. **프로젝트 템플릿** — 이 repo에서 **"Use this template"** 으로 새 프로젝트를 시작하면,
   스킬과 규칙이 그대로 따라가 **클라우드·로컬 어디서든 자동 로드**됩니다.
...
`.claude/skills/` 와 `.claude/rules/` 를 자기 repo로 다시 받아옵니다. 즉 창고만 고치면
하위 프로젝트들이 알아서 따라옵니다. (즉시 반영이 필요하면 그 repo의 Actions에서 워크플로를 수동 실행)
...
템플릿에서 시작하지 않은 기존 repo라면, 이 repo의 `.claude/skills/`·`.claude/rules/` 폴더와
`.github/workflows/sync-skills.yml` 을 복사해 커밋하면 동일하게 동작합니다.
(Claude Code 세션이면 "fitogether-claude-tools의 .claude/skills와 .claude/rules를 이 repo에 복사해줘" 라고 시키면 됩니다.)
