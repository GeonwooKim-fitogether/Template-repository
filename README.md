...
| 에이전트 | `kimengexpert` | 필요할 때 불러쓰는 **제품 내용 감수자 KEE(Kim Engineering Expert)**. 하드웨어·펌웨어·기구·양산이 만나는 이음새를 소유하고, 우리가 등록한 설계를 데이터시트·실측에 대조해 **근거를 인용하며 심문함** — 모든 판정에 출처(지도 블록·데이터시트 쪽수·시험 행)를 달고, 근거가 없으면 "모른다"를 두 가지(재료 없음 / 판단 갈림)로 갈라 말함. 설계·변경을 맡기면 일곱 칸짜리 **지적 카드**로, 물으면 **토론 상대**로 답함. 지식은 문서에 넣지 않고 저장소의 `docs/kee-*.md`에서 읽으며, 그것이 없는 저장소에서는 그 사실을 먼저 밝힘. 변경점 엔진 `drbfm-qa`와 겹치지 않음 — 그쪽은 변경점, 이쪽은 **변경이 없어도 도는 상시 규칙**까지 봄. 발언 효력은 현재 **자문**이며 최종 승인은 하지 않음. "kee 불러줘 / 이 설계 감수해줘"로 호출 |
