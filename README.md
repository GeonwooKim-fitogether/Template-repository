...
- **수정/추가**: `.claude/rules/<규칙>.md` 를 고치거나 새로 만들고 커밋. 새로 만들었으면 **위 표에 한 행을 함께 추가**합니다(표에 없는 규칙은 아무도 이 규칙이 있다는 것을 모릅니다).
...
Claude가 어떤 도구를 쓸 때 **"이 도구를 써도 될까요?" 창을 띄우지 않고 바로 실행하게 하는 목록**입니다. `.claude/settings.json` 의 `permissions.allow` 칸에 도구 이름을 적어 두면, 그 도구는 매번 묻지 않습니다.

현재 이 창고가 내려보내는 항목은 하나입니다.

| 허용 규칙 | 무엇이 자동으로 통과하나 | 왜 넣었나 |
|---|---|---|
| `mcp__Supabase__execute_sql` | Supabase 데이터베이스에 SQL 을 실행하는 도구 | 데이터를 확인하는 조회 한 번마다 권한 창이 떠서, 여러 번 나눠 조회하는 작업이 사람의 클릭을 계속 기다리며 멈춰 있었습니다. |

**여기에 한 가지 한계를 분명히 적어 둡니다.** MCP 도구의 허용 규칙은 **도구 하나를 통째로** 허용하거나 통째로 막는 것만 됩니다. 같은 `execute_sql` 안에서 `select`(조회)와 `delete`·`drop`(삭제)을 구분할 방법이 없습니다. 따라서 이 규칙을 켜 두면 **데이터를 바꾸거나 지우는 SQL 도 묻지 않고 실행됩니다.** 데이터베이스의 구조 자체를 바꾸는 `apply_migration` 은 이 목록에 넣지 않았으므로 여전히 사람에게 물어봅니다.

조회만 자동으로 통과시키고 나머지는 계속 묻게 하려면, 설정만으로는 안 되고 쿼리문을 읽어 판단하는 `PreToolUse` 훅을 따로 붙여야 합니다. 지금은 그 방식을 쓰지 않고 있습니다.
