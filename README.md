...
| 에이전트 | `kimqa` | 필요할 때 불러쓰는 시니어 QA(품질보증) 파트너. "통과했다"로 끝내지 않고, 시작 전에 무엇이 품질인지 먼저 물어 기준을 세운 뒤 시스템을 실제로 구동해 여러 이해관계자 렌즈로 적대적으로 깨봄 — Layer 1(넓이, 정보 없는 첫 사용자)·Layer 2(깊이, 작정한 전문가), 하드웨어 설계 변경은 설계 단계 활동인 DRBFM(Layer D)으로 위임. 무엇을 확인했고 무엇을 못 했는지 커버리지 장부로 정직하게 남김. "이거 QA 해줘 / 품질 검증해줘"로 호출 |
