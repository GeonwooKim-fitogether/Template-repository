...
| `deep-deliberation` | 기획·설계·방향 결정에서 성급한 수렴을 막고 발산→적대적 토론→종합→커버리지 게이트를 구조로 강제 |
...
| 에이전트 | `kimpm` | 필요할 때 불러쓰는 시니어 PM 파트너. 목표를 못 박고 판돈에 맞는 깊이로 팀 스킬을 지휘해 완결성 있는 결과를 냄. "KimPM 불러줘"로 호출 |
