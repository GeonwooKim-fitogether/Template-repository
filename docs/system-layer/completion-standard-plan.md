...
## 4. 결정 필요 — 대표가 답해야 갈리는 갈림길

각 항목에 권고안을 달았다. 답만 주시면 계획이 그대로 굴러간다.
...
## 남은 것 (이 계획이 검증하지 못한 부분)

- **동기화 봇의 실동작 미확인.** 창고 수정이 하위 저장소로 실제 내려가는지는 이번 검토 범위에서 확인하지 못했다. 1단계 머지 뒤 하위 저장소에서 규칙 7 확장본이 보이는지 확인해야 한다 — 확인 전까지 이 작업의 배포완료를 선언할 수 없다(새 기준의 자기 적용).
- **P1b 사고의 실제 커밋·PR 식별.** 8단계 소급 검증에 쓸 대상 커밋을 아직 특정하지 못했다. Hardware-Team-System의 이력에서 찾아야 한다.
- **고아 코드 검사의 오탐률.** 동적 import, 설정 기반 로딩(라우트가 파일 위치로 자동 등록되는 프레임워크) 등에서 grep 판정이 어긋날 수 있다. 결정 5의 권고(프롬프트로 시작해 스크립트로 승격)가 이 불확실성에 대한 대비다.
- **kimorchestrator·merge-gate의 심층 검토는 규칙 7 인용 여부까지만 확인했다.** 규칙 7을 고치면 따라온다고 판단했으나, 수정 후 한 번 훑어 옛 문구 잔재가 없는지 확인할 필요가 있다.
